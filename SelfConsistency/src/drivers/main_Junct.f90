program SelfConsistency
   ...
   !
   !PROJECT SPECIFIC>>>
   call dump_Matrix(diag(Crystal%Ek(:,1)),reg(pathINPUT),"Ediag.DAT")
   call dump_Matrix(dreal(Crystal%Zk(:,:,1)),reg(pathINPUT),"ReHrot.DAT")
   call dump_Matrix(dimag(Crystal%Zk(:,:,1)),reg(pathINPUT),"ImHrot.DAT")
   !
   call AllocateFermionicField(Glat,Crystal%Norb,Nmats,Nkpt=Crystal%Nkpt,Nsite=Nsite,Beta=Beta)
   call read_FermionicField(Glat,reg(ItFolder),"Glat_w",Crystal%kpt)
   do ik=1,Glat%Nkpt
      do iw=1,Glat%Npoints
         Glat%wks(:,:,iw,ik,1) = rotate(Glat%wks(:,:,iw,ik,1),Crystal%Zk(:,:,ik))
      enddo
      Glat%N_ks(:,:,ik,1) = rotate(Glat%N_ks(:,:,ik,1),Crystal%Zk(:,:,ik))
   enddo
   Glat%N_ks(:,:,:,Nspin) = Glat%N_ks(:,:,:,1)
   Glat%wks(:,:,:,:,Nspin) = Glat%wks(:,:,:,:,1)
   call FermionicKsum(Glat)
   call dump_Matrix(Glat%N_s,reg(ItFolder),"Ndig",paramagnet)
   call dump_FermionicField(Glat,reg(ItFolder),"Gdig_w",paramagnet)
   call dump_MaxEnt(Glat,"mats",reg(ItFolder)//"Convergence/","Gdig",EqvGWndx%SetOrbs,WmaxPade=PadeWlimit)
   call dump_MaxEnt(Glat,"mats2itau",reg(ItFolder)//"Convergence/","Gdig",EqvGWndx%SetOrbs)
